...
BEGIN TRY
	...
	DECLARE @PHI_DIEUTRI INT = (SELECT PHI FROM DIEUTRI WHERE ID_DIEUTRI = @ID_DIEUTRI);
	DECLARE @NGAYHOMNAY DATE = GETDATE();

	IF DBO.CHECK_TONTAI_THONGTIN_THANHTOAN(@ID_KEHOACH_DIEUTRI, @NGAYHOMNAY) = 0
	BEGIN
		EXEC TAO_THONGTIN_THANHTOAN 0, @PHI_DIEUTRI, @ID_KEHOACH_DIEUTRI, @NGAYHOMNAY
	END;
	...
	UPDATE KEHOACH_DIEUTRI
	SET TONG_CHIPHI = TONG_CHIPHI + @PHI_DIEUTRI
	WHERE ID_KEHOACH_DIEUTRI = @ID_KEHOACH_DIEUTRI

	-- CẬP NHẬT TỔNG CHI PHÍ CỦA THÔNG TIN THANH TOÁN
	UPDATE THONGTIN_THANHTOAN
	SET TONGTIENTHANHTOAN = TONGTIENTHANHTOAN + @PHI_DIEUTRI
	WHERE ID_KEHOACH_DIEUTRI = @ID_KEHOACH_DIEUTRI
