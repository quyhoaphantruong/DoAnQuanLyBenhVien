...
BEGIN
    ...
    SELECT KHDT.ID_KEHOACH_DIEUTRI, 
        (CASE 
            ...
            ELSE KHDT.TONG_CHIPHI - SUM(TT.TIENDATRA)
	...
	SELECT DT.ID_DONTHUOC, 
        (CASE 
            WHEN COALESCE(SUM(TT.TIENDATRA), 0) = 0 THEN DT.TONG_CHIPHI
            ELSE DT.TONG_CHIPHI - SUM(TT.TIENDATRA)
        END) AS TIEN_CONTHIEU
    FROM DONTHUOC DT
    LEFT JOIN THONGTIN_THANHTOAN_DONTHUOC TT ON DT.ID_DONTHUOC = TT.ID_DONTHUOC
    WHERE DT.ID_BENHNHAN = @ID_BENHNHAN
    GROUP BY DT.ID_DONTHUOC, DT.TONG_CHIPHI
END;
GO 
